...
BEGIN_MESSAGE_MAP(CSameGameView, CView)
    ON_WM_LBUTTONDOWN()
    ON_COMMAND(ID_Level_Easy, &CSameGameView::OnLevelEasy)
    ON_UPDATE_COMMAND_UI(ID_Level_Easy, &CSameGameView::OnUpdateLevelEasy)
    ON_COMMAND(ID_LEVEL_HARD, &CSameGameView::OnLevelHard)
    ON_UPDATE_COMMAND_UI(ID_LEVEL_HARD, &CSameGameView::OnUpdateLevelHard)
    ON_COMMAND(ID_LEVEL_NORMAL, &CSameGameView::OnLevelNormal)
    ON_UPDATE_COMMAND_UI(ID_LEVEL_NORMAL, &CSameGameView::OnUpdateLevelNormal)
END_MESSAGE_MAP()
...
void CSameGameView::ResizeWindows()
{
...
void CSameGameView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (count > 0)
    {
        ...
        if (pDoc->IsGameOver())
        {
            ...
            CString message;
            message.Format(_T("Không còn nước đi nào nữa !\nSố khối còn lại: %d"), remaining);
            //  Display the results to the user
            MessageBox(message, _T("Trò chơi kết thúc"), MB_OK | MB_ICONINFORMATION);
        }
    }
...
void CSameGameView::OnLevelEasy()
{
    //  First get a pointer to the document
    CSameGameDoc* pDoc = GetDocument();
    ASSERT_VALID(pDoc);
...
void CSameGameView::OnUpdateLevelEasy(CCmdUI* pCmdUI)
{
    //  First get a pointer to the document
    CSameGameDoc* pDoc = GetDocument();
    ASSERT_VALID(pDoc);
...
void CSameGameView::OnLevelHard()
{
    //  First get a pointer to the document
    CSameGameDoc* pDoc = GetDocument();
    ASSERT_VALID(pDoc);
...
void CSameGameView::OnUpdateLevelHard(CCmdUI* pCmdUI)
{
    //  First get a pointer to the document
    CSameGameDoc* pDoc = GetDocument();
    ASSERT_VALID(pDoc);
...
void CSameGameView::OnLevelNormal()
{
    //  First get a pointer to the document
    CSameGameDoc* pDoc = GetDocument();
    ASSERT_VALID(pDoc);
...
void CSameGameView::OnUpdateLevelNormal(CCmdUI* pCmdUI)
{
    //  First get a pointer to the document
    CSameGameDoc* pDoc = GetDocument();
    ASSERT_VALID(pDoc);
